document.addEventListener("DOMContentLoaded", () => {
	...
	async function renderForumScreen() {
		...
		if (globalFilters && globalFilters.length > 0) {
			groupsToRender = allGroups.filter((group) => group.categories && group.categories.some((cat) => globalFilters.includes(cat)));
		}
		// --- ▲▲▲ 新增结束 ▲▲▲ ---

		// 检查筛选后是否还有内容
		if (groupsToRender.length === 0) {
			const message = globalFilters.length > 0 ? "没有找到符合筛选条件的小组哦" : "还没有任何小组，点击右上角“+”创建一个吧！";
...
`;
	...
	async function deleteGroupAndPosts(groupId) {
		const group = await db.forumGroups.get(groupId);
		if (!group) return;
		const confirmed = await showCustomConfirm("确认删除", `确定要删除小组“${group.name}”吗？此操作将同时删除该小组内的【所有帖子和评论】，且无法恢复！`, { confirmButtonClass: "btn-danger" });
		if (confirmed) {
			try {
...
`;
	...
	document.getElementById("group-post-list").addEventListener("click", async (e) => {
		...
		if (e.target.classList.contains("forum-post-delete-btn")) {
			...
			const confirmed = await showCustomConfirm("删除帖子", `确定要删除帖子《${post.title}》吗？此操作将同时删除帖子下的所有评论，且无法恢复。`, { confirmButtonClass: "btn-danger" });
