...
const GENERIC_PRODUCT_PROMPTS = [
    {
        ...
        prompt: "A piece of modern clothing, {productName}, elegantly displayed on a mannequin or lying flat, clean minimalist studio shot, professional product photography, soft shadows, solid color background, high detail, photorealistic, 8k",
    },
    {
        keywords: ["鞋", "靴", "sneaker", "boot"],
        englishCategory: "a modern sneaker",
        prompt: "A single modern shoe, {productName}, studio product shot, minimalist, on a solid color platform, detailed, photorealistic, commercial photography, soft studio lighting",
    },
    {
        keywords: ["包", "袋", "backpack", "handbag"],
        englishCategory: "a stylish modern bag",
        prompt: "A stylish modern bag, {productName}, professional product photography, minimalist, clean background, studio lighting, high fashion, high detail, 8k, hyperrealistic",
    },
    {
        keywords: ["手机", "耳机", "键盘", "鼠标", "数据线", "充电", "数码", "电子"],
        englishCategory: "a sleek electronic gadget",
        prompt: "A sleek electronic gadget, {productName}, on a clean modern desk, minimalist product shot, tech aesthetic, studio lighting, photorealistic, octane render, 8k",
    },
    {
        keywords: ["零食", "饼干", "薯片", "糖", "巧克力", "水", "饮料", "茶", "咖啡", "food", "snack", "面", "饭", "汉堡", "奶茶"],
        englishCategory: "a delicious-looking food or drink product",
        prompt: "Delicious-looking {productName}, professional product shot, appetizing, packaging or food itself displayed, minimalist setup, vibrant colors, solid color background, high detail, food photography",
    },
    {
        keywords: ["杯", "碗", "盘", "锅", "灯", "毯", "枕", "家居", "摆件", "装饰"],
        englishCategory: "a modern home decor item",
        prompt: "A modern home decor item, {productName}, in a cozy and minimalist living room setting, soft lighting, professional product shot, high detail, photorealistic, interior design magazine style",
    },
    {
        keywords: ["化妆", "护肤", "口红", "眼影", "香水", "面膜"],
        englishCategory: "a cosmetic product package",
        prompt: "A bottle or package of a cosmetic product, {productName}, clean product shot, on a podium with simple geometric shapes, minimalist, beauty photography, high detail, solid color background, soft shadows",
    },
    {
        keywords: ["玩具", "玩偶", "模型", "手办", "toy", "figure"],
        englishCategory: "a cute or cool toy figure",
        prompt: "A cute or cool toy figure, {productName}, product shot, on a simple stand, plain background, studio lighting, detailed, collectible, anime style, 8k",
    },
    {
        keywords: ["书", "本", "笔", "文具"],
        englishCategory: "a book or stationery item",
        prompt: "A book or stationery item, {productName}, neatly arranged on a clean desk, minimalist, flat lay photography, high detail, studio lighting, soft shadows",
    },
    {
        ...
        // ★★★ 全面优化的备用提示词 ★★★
        prompt: "Commercial product photography of {productName}. Professional studio shot, clean minimalist aesthetic, displayed on a podium or flat surface. Soft, even lighting, subtle soft shadows. Shot on a high-end camera, 8k, hyperrealistic, high detail.",
    },
];
...
function getRandomWaimaiImage() {
    const defaultImages = ["https://i.postimg.cc/mD8DB9Q7/food1.jpg", "https://i.postimg.cc/W12WqgJp/food2.jpg", "https://i.postimg.cc/KzA1df4y/food3.jpg"];
...
async function clearTaobaoProducts() {
    ...
    const confirmed = await showCustomConfirm("确认清空", "确定要清空桃宝首页的所有商品吗？此操作将【一并清空购物车】，且无法恢复。", { confirmButtonClass: "btn-danger" });
...
async function renderElemeFoods() {
    ...
    if (foods.length === 0) {
        gridEl.innerHTML = '<p style="grid-column: 1 / -1; text-align: center; color: var(--text-secondary);">还没有美食哦，点击右上角“✨”或“+”来发现美味吧！</p>';
        return;
    }
...
async function openCharSelectorForEleme() {
    return new Promise((resolve) => {
        ...
        singleChats.forEach((chat) => {
            ...
            item.innerHTML = `
                <input type="radio" name="eleme-target" value="${chat.id}" id="target-${chat.id}" style="margin-right: 15px;">
...
async function sendElemeOrderNotificationToChar(targetChatId, food, remark) {
    ...
    // 准备给AI看的文本，现在包含了备注信息
    const textContentForAI = `[系统提示：用户给你点了一份来自“${food.restaurant}”的外卖“${food.name}”，并备注说：“${remark || "无"}”。请根据你的人设对此作出回应。]`;
...
async function handleBuyProduct(productId) {
    ...
    const confirmed = await showCustomConfirm("确认购买", `确定要花费 ¥${product.price.toFixed(2)} 购买“${product.name}”吗？`, { confirmText: "立即支付" });
...
async function handleShareCart() {
    ...
    const confirmed = await showCustomConfirm("确认代付", `将分享购物车给“${char.name}”并请求代付，共计 ¥${totalPrice.toFixed(2)}。\n这将会清空你的购物车，并从Ta的钱包扣款。确定吗？`, { confirmText: "确定" });
...
async function handleBuyForChar() {
    ...
    const confirmed = await showCustomConfirm("确认赠送", `确定要花费 ¥${totalPrice.toFixed(2)} 为“${char.name}”购买购物车中的所有商品吗？`, { confirmText: "为Ta买单" });
...
async function handleShareCartRequest() {
    ...
    const confirmed = await showCustomConfirm("确认代付请求", `将向“${chat.name}”发起购物车代付请求，共计 ¥${totalPrice.toFixed(2)}。`, { confirmText: "发送请求" });
    ...
    document.getElementById("taobao-screen").addEventListener("click", async (e) => {
        ...
        if (target.closest("#products-view")) {
            ...
            // 如果点击的是分类页签
                ...
                const category = target.closest(".category-tab-btn").dataset.category === "all" ? null : target.closest(".category-tab-btn").dataset.category;
        ...
        // --- 购物车页 ---
            ...
            if (target.closest(".cart-item-info") || target.classList.contains("product-image")) {
                ...
                if (cartItem) {
                    ...
                    const productId = parseInt(cartItem.querySelector(".delete-cart-item-btn").dataset.productId) || parseInt(target.dataset.productId);
                    await openProductDetail(productId);
                }
